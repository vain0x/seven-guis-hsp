...
- テキストボックス **Tp** があり、これに入力されている文字列を名前に持つ人物で、リストを絞り込む
    - 入力ごとに絞り込みを行う (エンターキーを押したときとかではなく)
- テキストボックス **Tn**, **Ts** があり、それぞれ人物の姓・名を入力する
- ボタンが3つある:
    - ボタン **Bc** (Create) を押すと、Tn, Ts に入力されている名前で人物を1件登録する
    - ボタン **Tu** (Update) を押すと、リストで選択中の人物の名前を、Tn, Ts に入力されている名前に更新する
    - ボタン **Td** (Delete) を押すと、リストで選択中の項目を削除する
...
- データの持ちかた:
    - 人物データの件数を `person_count` 変数で持ち、i番目の人物の名前・姓をそれぞれ person_names(i), person_surnames(i) で持つ
    - リストをクリックしたとき、その人物のデータはこれらの配列を参照する
    - Createなどの処理はこれらの配列を変更し、同時にUIに反映する
- ボタンの処理:
    - 作成(Create): 入力されている姓名をpersons配列に追加する。リストに項目を追加し、それを選択状態にする
        - 選択後、姓名の入力欄を更新する
    - 更新(Update): 入力されている姓名をpersons配列に設定し、リストの項目を更新する
        - リスト項目のテキストを更新するためのメッセージはないので、その項目を削除し、同じ位置に新しいテキストを持つ項目を挿入し、それを選択状態にする
    - 削除(Delete): 選択されている項目をpersons配列から削除する。リストからも同じ位置の項目を削除し、代わりに近い位置にある項目を選択する
        - 配列の途中の要素の削除はややこしい (その位置以降で末尾以外の各要素につき、要素の値を次の要素の値で上書きする、という手順で更新できる)
        - 選択状態の更新:
            - リストの末尾の項目を削除した場合、項目が1つ以上あるなら、その1つ前の項目を選択する。削除後にリストが空になる場合、選択はしない
            - 先頭または中間の項目を削除した場合、同じインデックスの項目を選択する
            - 選択後、姓名の入力欄を更新する

- TODO: フィルターの実装、ウィンドウサイズ変更時のレイアウト更新、PDS
