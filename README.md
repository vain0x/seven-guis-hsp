...
- 課題:
    - キャンバスの端にある円がキャンバスからはみ出してしまう
        - キャンバス内を別のバッファーに書き込んでgcopyでコピーすればいい
    - 円が重なっているときに選択中の円が「手前」にこない (ほかの円が上から重ねて描画されてしまう)
        - 選択中の円を最後に描画する
- undo/redo:
    - undo可能な操作は次の2つ
        - 円の追加(add)、円のサイズ変更(resize)
    - redoする場合に円の削除(delete)という新しい操作が必要になる
        - `circle_deleted(i)` で円が削除状態を持つ (0: 未削除, 1: 削除済み)。削除済みの円は無視する (描画されず、選択状態にならない)
    - changes配列を用意する
        - `undo_count` はundo可能な操作の回数を表し、`redo_count` はredo可能な操作の回数を表す
        - `change_type` という配列に操作の種類が入っている (1: 円の追加, 2: サイズ変更)
        - `change_type` の前方 `undo_count` 個と、それに続く `redo_count` 個がそれぞれ操作を表している
            (つまり `(undo_count + redo_count)` 個の有効な要素を持つ)
            (TODO: 図解)
        - undo可能な操作が行われるたびに、`change_type` の `undo_count` 個の要素の後に操作データが追加される
            - 円の追加なら、追加した円の番号を記録する (`change_ci`)
            - 円のサイズ変更なら、変更された円の番号と、変更前のサイズを記録する (`change_ci` と `change_diam`)
                - 操作の種類によって関連する配列のうち利用するものが変化することに注意。
                    円の直径の値 (`change_diam`) は、その操作が円のサイズ変更の場合にのみ参照できる
            - `redo_count` を0にする (redoできなくするため。実質的にchanges配列が縮む)
    - undo操作: `undo_count >= 1` とする (0なら何もできない)。`change_type(undo_count - 1)` が直前の操作なので、これを取り消す
        - 取り消し処理は操作の種類による:
            - 円の追加なら、その円を削除状態にする
            - 円のサイズ変更なら、その円の直径を変更前の値に戻す
        - `undo_count` は1減らし、`redo_count` を1増やす。トータルとして配列の長さは変わらないが、「undo側」にあった要素が「redo側」に移動する
    - redo操作: `redo_count >= 1` とする (0なら何もできない)。`change_type(undo_count)` が最後にundoされた操作なので、これを再適用する
        - 再適用処理は操作の種類による:
            - 円の追加なら、削除状態になったその円を未削除の状態に戻す
