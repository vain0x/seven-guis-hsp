...
[circle.hsp](circle.hsp)

### ターン1

- (仕様が大きくなってきたので分割して実装した。undo/redoはめんどくさいので後回しにする)
- undo/redoボタンを置く、キャンバスの部分をクリックして円を置く、円を選択できる、ところだけやる
- データの持ちかた:
    - circle配列を用意する (CRUDの人物と同様)
    - 円の個数を `circle_count` で持ち、i番目の円のX・Y座標をそれぞれ `circle_x(i)`, `circle_y(i)` で持ち、直径を `circle_diam(i)` で持つ
- スクリーン内のレイアウトを計算してキャンバスの位置・大きさを決める
- メインループ:
    - (いままでのタスクはstop命令で処理を止めていた が、今回はメインループを回す。onclickやWM_MOUSEMOVEを使えばイベント駆動にもできるはず)
    - マウスの位置: システム変数 `mousex`, `mousey` でウィンドウ内におけるマウスの位置を取得できる。ただし、これらのシステム変数はマウスがのウィンドウ外にある状態では正確な値にならない。マウスがウィンドウ内にあることを判定し、その場合のみマウスの位置を取得する
    - キャンバス内のマウスの位置: このアプリでは「キャンバスの左上隅」を基準とするマウスの位置のみ知りたいので、キャンバスの左上を基準とする値に変換する。マウスがキャンバス外にある場合、マウス位置は無視していい
    - 左クリックの検出: stick命令で左クリックを検出し、クリックされていたら円の追加を行う
    - 円の選択:
        - 円の中心点とマウスの(キャンバス内の)位置の距離を計算する
        - 円と点の衝突判定 (`(x-a)^2 + (y-a)^2 <= r^2`) を使って、それぞれの円についてマウスがその上に乗っているか判定する
        - マウスが上に乗っている円の中で、マウスとの距離が最小のものを1つ選んで選択状態とする
    - 円を描画する
- この時点でundo/redoボタンは何もしない。円を右クリックしても何も起きない
- 描画に関する問題が2つある (詳細はターン2)

### ターン2

- 次に、前項の問題であるキャンバスの描画を修正した
- (P1) 円をキャンバス内に直接描画すると、キャンバスの端にある円がはみ出してしまう
    - 解決: キャンバス内のコンテンツをバッファー(buffer)に描いてgcopyでコピーしてくればいい
- (P2) 円が重なっているときに選択中の円が「手前」にこない (ほかの円が上から重ねて描画されてしまう)
    - 解決: 選択中の円を最後に描画する
    - 円の描画順を配列として持つ (描画時に生成する)。選択状態の円があるなら交換する

### ターン3

- 次に、円のサイズ変更操作を作った
- 編集ダイアログを作る
    - screen命令のオプションを指定して非表示、固定サイズ、ツールウィンドウにする
    - トラックバーを配置する (Timerを参照)
- メインループで選択状態の円があって、(stick命令によって)右クリックを検出したら
- 編集の開始:
    - 編集モード (`editing == 1`) にして、編集中の円のインデックスを持つ
    - トラックバーの位置を直径に合わせて、編集ダイアログを表示する
- 編集の終了:
    - onexit命令でウィンドウが閉じられる前に処理できるようにする
    - 編集ダイアログが閉じられる場合、代わりに編集の終了処理をする
    - (メインウィンドウが閉じられる場合、アプリの終了処理を行う)
- モーダル性: screen命令で作ったウィンドウは「モーダル」ではない。つまり、そのウィンドウが開いている間にほかのウィンドウも操作できる (dialog命令なら「モーダル」なので操作できない)
    - 編集中に円の追加やundoなどが行われるとややこしいので、モーダルにしたい (そういう仕様のはず)
    - メインループの開始時にアクティブなウィンドウが編集ダイアログでなければ、自動的に編集ダイアログをアクティブにしなおすという処理を入れる (この解決策がいいのか不明)

### ターン4

- 最後に、undo/redoを実装する
- undo可能な操作 (編集操作) は次の2つ
    - 円の追加(add)
    - 円のサイズ変更(resize)
- redoする場合に円の削除(delete)という新しい操作が必要になる
    - `circle_deleted(i)` で円の削除状態を持つ (0: 未削除, 1: 削除済み)
    - 削除済みの円は無視する (描画されず、選択状態にならない)
- change配列を用意する (後に図解がある)
    - `undo_count` はundo可能な編集操作の回数を表し、`redo_count` はredo可能な回数を表す
    - 配列の前方 `undo_count` 個に適用済みの操作が入り、それに続く `redo_count` 個にundo済みの操作が入る
    - `change_type(i)` は操作の種類を持つ
    - 編集操作の際に、`change_type` の前方 `undo_count` 個の要素の後に、操作データを追加する
        - 円の追加なら、追加された円の番号も記録する (`change_ci`)
        - 円のサイズ変更なら、変更された円の番号と、変更前のサイズを記録する (`change_ci`, `change_diam`)
        - 操作の種類によって関連する配列のうち利用するものが異なることに注意。
            円の直径の値 (`change_diam`) は、その操作が円のサイズ変更の場合にのみ参照できる
        - 同時に、`redo_count` を0にする。実質的にchange配列の後半を捨てて、redoできなくするということ
- undo操作: `undo_count >= 1` の場合 (0なら何もできない)、`change_type(undo_count - 1)` が直前の操作なので、これを取り消す
    - 取り消し処理は操作の種類による:
        - 円の追加なら、その円を削除状態にする
        - 円のサイズ変更なら、その円の直径を変更前の値に戻す
    - `undo_count` を1減らし、`redo_count` を1増やす。トータルとして配列の長さは変わらないが、「undo側」にあった要素が「redo側」に移動する
- redo操作: `redo_count >= 1` の場合 (0なら何もできない)、`change_type(undo_count)` が最後にundoされた操作なので、これを再適用する
    - 再適用処理は操作の種類による:
        - 円の追加なら、削除状態になっているその円を未削除の状態に戻す
        - 円のサイズ変更なら、その円の直径を変更後の値にする
    - `undo_count` を1増やし、`redo_count` を1減らす。この場合もトータルとして配列の長さは変わらず、「redo側」にあった操作が「undo側」に移動する

#### change配列の図

「円の追加、円の追加、サイズ変更、円の追加、円の追加、undo、undo」という操作を行うと、change配列は次の状態になる:

```
                 undo_count=3 redo_count=2
    <-----------------------> <------------->
    +-------+-------+--------+-----+--------+
    |       |       |        |     |        |
    | add   | add   | resize | add | resize |
    |       |       |        |     |        |
    +-------+-------+--------+-----+--------+
                             ^
                             |
...
2回のundoにより、アプリの状態は「円の追加、円の追加、サイズ変更」(undo_countの下にある3つの操作)が反映された状態になっている

- (A) 次にundo操作をするなら、「current↑」 が左に1つ移動し、サイズ変更が取り消される
- (B) 次にredo操作をするなら、「current↑」が右に1つ移動し、円の追加が再適用される
- (C) 次にundo可能な操作をするなら、「current↑」の右上にあるaddのところに次の操作が上書きされて、さらに右にあるresizeのところが捨てられる
